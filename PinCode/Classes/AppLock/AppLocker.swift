...
public typealias onCancelCallback = (_ mode: ALMode) -> ()
...
public struct ALAppearance { // The structure used to display the controller
    ...
    public var onCancelAttempt: onCancelCallback?
...
public class AppLocker: UIViewController {
    ...
    private var onCancelAttempt: onCancelCallback?
    ...
    @IBAction func keyboardPressed(_ sender: UIButton) {
        ...
        case ALConstants.button.cancel.rawValue:
            clearView()
            if let action = self.onCancelAttempt {
                action(self.mode)
                dismiss(animated: true)
                return
            }
...
public extension AppLocker {
    ...
    class func present(with mode: ALMode, and config: ALAppearance? = nil) {
        ...
        locker.onCancelAttempt = config?.onCancelAttempt
